...
export default function Home() {
  ...
  const [color, setColor] = useState<RGBColor>(randomColors[randomColors.length * Math.random() | 0]);
  ...
  const textColor = getColorBrightness(color) > 145 ? 'text-neutral-800' : 'text-neutral-200';
      ...
      style={{
        ...
        <div className='h-full flex flex-col items-center justify-center gap-5'>
          <div className='flex flex-row items-center justify-center gap-5'>
            <RgbColorPicker color={color} onChange={onPickerChange} style={{
              width: '400px',
              height: '300px',
            }}/>
            <div className='flex flex-col gap-2'>
              <div className='flex flex-row gap-2'>
                <ColorInfo textColor={textColor} title='HEX' value={colorHEX} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {setColor(HEXtoRGB(event.target.value)); setColorHEX((event.target.value.includes('#') ? '' : '#') + event.target.value)}} maxLength={7} />
                <ColorInfo textColor={textColor} title='DEC' value={colorToDEC(color).toString() || '0'} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {setColor(DECToColor(parseInt(event.target.value))); setColorHEX((event.target.value.includes('#') ? '' : '#') + event.target.value)}} maxLength={8} />
              </div>
              <RGBColorInfo color={color} onChange={setColor} textColor={textColor} />
